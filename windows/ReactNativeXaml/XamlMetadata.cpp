    ...
    ExecuteJsi(ctx, [shared = shared_from_this()](facebook::jsi::Runtime &rt) {
      ...
      if (!batchedBridge.isUndefined() && batchedBridge.isObject()) {
        if (auto vm = shared.get()) {
          ...
          vm->m_eventEmitter =
              getCallableModule.callWithThis(rt, batchedBridge.asObject(rt), "RCTEventEmitter").asObject(rt);
...
void SetIsOpen_FlyoutBase(
    const xaml::DependencyObject &o,
    const xaml::DependencyProperty &,
    const winrt::Microsoft::ReactNative::JSValue &v,
    const winrt::Microsoft::ReactNative::IReactContext &context) {
  const auto &xaml = context.Properties()
                         .Get(ReactNativeXaml::XamlViewManager::XamlViewManagerProperty().Handle())
                         .as<ReactNativeXaml::XamlViewManager>();
  ...
  if (auto cd = depObj.try_as<ContentDialog>()) {
    ...
    op.Completed([cd, context](const IAsyncOperation<ContentDialogResult> &operation, const AsyncStatus &asyncStatus) {
      if (asyncStatus == AsyncStatus::Completed) {
        ...
        XamlUIService::FromContext(context).DispatchEvent(
            cd,
            L"topContentDialogClosed",
...
xaml::DependencyProperty GetPriorityProperty() {
  static const auto priority = DependencyProperty::RegisterAttached(
      L"Priority", winrt::xaml_typename<uint32_t>(), viewPanelTypeName, xaml::PropertyMetadata{winrt::box_value(0u)});
  return priority;
}

void SetPriority_DependencyObject(
    const DependencyObject &u,
    const xaml::DependencyProperty &,
    const winrt::Microsoft::ReactNative::JSValue &v,
...
const PropInfo *XamlMetadata::FindFirstMatch(
    const stringKey &key,
...
const struct {
  const PropInfo *map;
  size_t size;
} propertyMaps[] = {{xamlPropertyMap, ARRAYSIZE(xamlPropertyMap)}, {fakeProps, ARRAYSIZE(fakeProps)}};

const PropInfo *XamlMetadata::GetProp(
...
const EventInfo *XamlMetadata::AttachEvent(
    const winrt::Microsoft::ReactNative::IReactContext &context,
    const std::string &propertyName,
    const winrt::Windows::Foundation::IInspectable &obj,
    bool attaching) {
  // obj is always a FrameworkElement (either a control or a wrapped control)
  if (!propertyName._Starts_with("on"))
    return nullptr;
  ...
  auto attachedEvt = std::find_if(
      wrapper->second.events.cbegin(), wrapper->second.events.cend(), [&evtName](AttachedEventInfo const &ei) {
        return ei.name == evtName;
      });
...
void XamlMetadata::JsiDispatchEvent(
    jsi::Runtime &rt,
    int64_t viewTag,
    std::string &&eventName,
    std::shared_ptr<facebook::jsi::Object> &eventData) const noexcept {
...
winrt::IAsyncOperation<winrt::Windows::Storage::Streams::InMemoryRandomAccessStream> GetImageInlineDataAsync(
...
winrt::fire_and_forget winrt::Microsoft::ReactNative::SetImageSourceForInlineData(
    std::string str,
    xaml::DependencyObject o,
...
void XamlMetadata::DispatchCommand(
    FrameworkElement const &view,
    winrt::hstring const &commandId,
    const winrt::Microsoft::ReactNative::JSValueArray &args) const noexcept {
  const std::string name = winrt::to_string(commandId);
  auto it = std::find_if(xamlCommands, xamlCommands + std::size(xamlCommands), [name](const XamlCommand &entry) {
    return Equals(entry.name, name.c_str());
  });
