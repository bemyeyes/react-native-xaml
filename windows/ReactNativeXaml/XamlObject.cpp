...
const EventArgsProperty *GetProp(const std::string &name, const winrt::Windows::Foundation::IInspectable &obj) {
  auto it = std::find_if(
      eventArgsProperties, eventArgsProperties + ARRAYSIZE(eventArgsProperties), [name](const EventArgsProperty &e) {
        return e.name == name;
      });
...
jsi::Value Windows_UI_Xaml_Input_ContextRequestedEventArgs_TryGetPosition(
    jsi::Runtime &rt,
    std::shared_ptr<XamlObject> thisVal,
...
const EventArgsMethod *GetEventArgsMethod(
    const std::string &name,
    const winrt::Windows::Foundation::IInspectable &obj) {
  auto it =
      std::find_if(eventArgsMethods, eventArgsMethods + ARRAYSIZE(eventArgsMethods), [name](const EventArgsMethod &e) {
        return e.name == name;
      });
...
facebook::jsi::Value XamlObject::IInspectableToValue(
    jsi::Runtime &rt,
    const winrt::Windows::Foundation::IInspectable &o) const {
  if (o == nullptr)
  ...
  if (auto refStr = o.try_as<winrt::Windows::Foundation::IReference<winrt::hstring>>()) {
    ...
    auto fontFamily = RunOnUIThread([&]() { return ff.Source(); });
