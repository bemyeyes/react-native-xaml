...
struct XamlObject : std::enable_shared_from_this<XamlObject>, facebook::jsi::HostObject {
  ...
  void set(
      ...
      const facebook::jsi::Value &value) noexcept override;
  std::vector<facebook::jsi::PropNameID> getPropertyNames(facebook::jsi::Runtime &rt) noexcept override;

  XamlObject() = default;
  XamlObject(winrt::Windows::Foundation::IInspectable obj, std::shared_ptr<const XamlMetadata> metadata)
      : m_obj(obj), m_metadata(metadata) {}
  XamlObject(const XamlObject &) = default;
  template <typename T = winrt::Windows::Foundation::IInspectable>
  T try_as() const {
    return m_obj.try_as<T>();
  }

  facebook::jsi::Value IInspectableToValue(
      facebook::jsi::Runtime &rt,
      const winrt::Windows::Foundation::IInspectable &inspectable) const;
